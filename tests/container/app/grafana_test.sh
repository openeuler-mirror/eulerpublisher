...
test_grafana() {
    container=$(docker_run_server -p "3000:3000")
    log="HTTP Server Listen"
    wait_container_ready "${container}" "${log}" > /dev/null
    ret=$(curl http://localhost:3000 2>&1)
    assertNotNull "$(echo $ret | grep "<a href=\"/login\">Found</a>")"
}
