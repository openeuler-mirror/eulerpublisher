#!/usr/bin/env bash
#即为官方文档中的脚本curl -sL https://raw.githubusercontent.com/slimtoolkit/slim/master/scripts/install-slim.sh | sudo -E bash -
function get_slim() {
  local DIST=""
  local EXT=""
  local FILENAME=""
  ...
  if [ -n "$1" ]; then
    ...
    #通过GitHub API获取Slim的最新发布版本号并赋值给`VER`。
  ...
  if [ -n "${VER}" ]; then
  #专门有个网站下载slim的压缩包，而不是直接从GitHub下载。
