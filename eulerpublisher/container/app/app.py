...
from eulerpublisher.publisher import EP_PATH, logger, DEFAULT_APP_ARCHES, AVAILABLE_ARCHES
...
class AppPublisher(pb.Publisher):
    def __init__(
        ...
        # architectures of required image, default is multi-platform
        arches = DEFAULT_APP_ARCHES.keys()
        if arch:
            arches = arch.replace(" ", "").split(",")
        platforms = [f"linux/{AVAILABLE_ARCHES[a]}" for a in arches]
        self.platform = ",".join(platforms)

        # workdir
        if ("EP_APP_WORKDIR" not in os.environ) or (not os.environ["EP_APP_WORKDIR"]):
